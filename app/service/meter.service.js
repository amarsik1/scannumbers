...
const update = async (meter) => {
    const {id, personal_account, resource_type_id, organization_id, meters_group_id, name} = meter;
    await pool.query("UPDATE meter SET personal_account = $1, resource_type_id = $2, organization_id = $3, meters_group_id = $4, name = $5 where meter_id = $5",
        [personal_account, resource_type_id, organization_id, meters_group_id, id, name]);
};
