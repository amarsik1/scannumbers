...
const create = async (req, res) => {
    ...
    if (!innerMeter) return res.status(400).send('Meter is not exist');
    const { value: prevValue } = await meterDataService.getLastRowFromMeter(meter_id);
    const date = new Date();
    if (prevValue) {
        if (prevValue > value) return res.status(400).send('The value cannot be less than the previous');
        if ((value - prevValue) > difference[innerMeter.resource_type]) return res.status(400).send('Value is much higher than the previous');
    }
