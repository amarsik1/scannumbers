...
const getOnce = async (req, res) => {
    ...
    if (!address) return res.status(400).send({ message: 'Address does not exists' });
...
const create = async (req, res) => {
    ...
    if (await addressService.isValueExist(req.body)) return res.status(400).send({ message: 'Address with this value already exists' });
...
const deleteOnce = async (req, res) => {
    const { id } = req.params;
    if (!await addressService.isExist(id)) return res.status(400).send({ message: 'Address does not exists' });
    const responce = await addressService.deleteOne(id);

    if (responce.type === 'ok')
        res.status(200).send({ message: "Was success deleted" });
...
const update = async (req, res) => {
    ...
    res.status(200).send({ message: "Address was updated successfully" });
};
